...
def process_invoice_image(image_file) -> list:
    ...
    try:
        ...
        if not items:
            diag = (
                f"Langs: {available_langs} | Usando: {use_lang} | Errores: {'; '.join(errors)}"
                if errors
                else f"Langs: {available_langs} | Usando: {use_lang}"
            )
...
def parse_invoice_text(text: str) -> list:
    ...
    p2 = re.compile(r"^(.{3,80}?)\s+" r"(\d+[.,]?\d*)\s+" rf"{_cur}\s*(\d[\d.,]{{3,}})\s*{_cur}\s*$")
    # Pattern 3: qty first then name then prices
    p3 = re.compile(
        r"^(\d+[.,]?\d*)\s+" r"(.{3,80}?)\s+" rf"{_cur}\s*(\d[\d.,]*)\s*{_cur}\s+" rf"{_cur}\s*(\d[\d.,]*)\s*{_cur}\s*$"
    )
    # Pattern 4: name followed by a price (single item)
    p4 = re.compile(r"^(.{5,60}?)\s+" rf"{_cur}\s*(\d[\d.,]{{3,}})\s*{_cur}\s*$")
