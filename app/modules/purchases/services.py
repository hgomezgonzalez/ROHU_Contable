...
def _next_po_number(tenant_id: str) -> str:
    ...
    last = (
        ...
        .filter(PurchaseOrder.tenant_id == tenant_id, PurchaseOrder.order_number.like(f"{prefix}%"))
        .scalar()
    )
...
def receive_purchase_order(
    ...
    for po_item in po.items:
        ...
        if received_items:
            match = next((r for r in received_items if r.get("item_id") == str(po_item.id)), None)
            if match:
        ...
        product = Product.query.filter_by(id=po_item.product_id, tenant_id=tenant_id).with_for_update().first()
    ...
    if total_cost > 0:
        ...
        lines = [
            {"puc_code": "1435", "debit": inventory_debit, "credit": 0, "description": f"Compra {po.order_number}"},
        ]
        if not is_simplified and total_tax > 0:
            lines.append({"puc_code": "2408", "debit": float(total_tax), "credit": 0, "description": "IVA descontable"})
        ...
        create_journal_entry(
            ...
            created_by=user_id,
            entry_type="PURCHASE",
            description=f"Recepción {po.order_number}",
            lines=lines,
            source_document_type="purchase_order",
            source_document_id=str(po.id),
        )
...
def list_purchase_orders(
    ...
    total = q.count()
    orders = q.order_by(PurchaseOrder.order_date.desc()).offset((page - 1) * per_page).limit(per_page).all()
...
def _next_payment_number(tenant_id: str) -> str:
    ...
    last = (
        ...
        .filter(SupplierPayment.tenant_id == tenant_id, SupplierPayment.payment_number.like(f"{prefix}%"))
        .scalar()
    )
    seq = int(last.split("-")[-1]) + 1 if last else 1
    return f"{prefix}{seq:06d}"
...
def create_supplier_payment(
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": "2205", "debit": float(amt), "credit": 0, "description": f"Pago a {supplier.name}"},
            {
...
def void_supplier_payment(tenant_id: str, payment_id: str, user_id: str) -> dict:
    ...
    payment = SupplierPayment.query.filter_by(id=payment_id, tenant_id=tenant_id).first()
    if not payment:
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": cash_account, "debit": float(payment.amount), "credit": 0, "description": "Reversa egreso"},
            {"puc_code": "2205", "debit": 0, "credit": float(payment.amount), "description": "Reversa pago proveedor"},
        ],
        source_document_type="supplier_payment_void",
        source_document_id=str(payment.id),
    )
...
def _next_pcn_number(tenant_id: str) -> str:
    ...
    last = (
        ...
        .filter(PurchaseCreditNote.tenant_id == tenant_id, PurchaseCreditNote.note_number.like(f"{prefix}%"))
        .scalar()
    )
    seq = int(last.split("-")[-1]) + 1 if last else 1
    return f"{prefix}{seq:06d}"
...
def create_purchase_credit_note(
    ...
    cn = PurchaseCreditNote(
        ...
        created_by=created_by,
        purchase_order_id=purchase_order_id,
        note_number=_next_pcn_number(tenant_id),
        reason=reason,
        subtotal=0,
        tax_amount=0,
        total_amount=0,
...
def _next_pdn_number(tenant_id: str) -> str:
    ...
    last = (
        ...
        .filter(PurchaseDebitNote.tenant_id == tenant_id, PurchaseDebitNote.note_number.like(f"{prefix}%"))
        .scalar()
    )
    seq = int(last.split("-")[-1]) + 1 if last else 1
    return f"{prefix}{seq:06d}"
...
def create_purchase_debit_note(
    ...
    dn = PurchaseDebitNote(
        ...
        created_by=created_by,
        purchase_order_id=purchase_order_id,
        note_number=_next_pdn_number(tenant_id),
        reason=reason,
        amount=amt,
        tax_amount=tax,
        total_amount=total,
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": "5195", "debit": float(total), "credit": 0, "description": reason},
            {"puc_code": "2205", "debit": 0, "credit": float(total), "description": f"Cargo proveedor {supplier.name}"},
        ],
        source_document_type="purchase_debit_note",
