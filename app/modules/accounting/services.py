...
def get_or_create_period(tenant_id: str, date: datetime = None) -> AccountingPeriod:
    ...
    period = AccountingPeriod.query.filter_by(tenant_id=tenant_id, year=date.year, month=date.month).first()

    if not period:
        period = AccountingPeriod(tenant_id=tenant_id, year=date.year, month=date.month)
...
def close_period(tenant_id: str, year: int, month: int, user_id: str) -> dict:
    ...
    period = AccountingPeriod.query.filter_by(tenant_id=tenant_id, year=year, month=month).first()
    if not period:
...
def create_journal_entry(
    ...
    if period.status == "closed" and entry_type not in ("CLOSING", "REVERSAL"):
        raise ValueError(
            f"Periodo {period.year}-{period.month:02d} está cerrado. Reabra el periodo para registrar movimientos."
        )
    ...
    for line_data in lines:
        ...
        if line_data.get("account_id"):
            account = ChartOfAccount.query.filter_by(id=line_data["account_id"], tenant_id=tenant_id).first()
        elif line_data.get("puc_code"):
            account = ChartOfAccount.query.filter_by(tenant_id=tenant_id, puc_code=line_data["puc_code"]).first()
        if not account:
    ...
    if total_debit != total_credit:
        raise ValueError(f"Asiento desbalanceado: débitos={total_debit}, créditos={total_credit}")
...
def post_sale_entry(
    ...
    entry1 = create_journal_entry(
        ...
        created_by=created_by,
        entry_type="SALE",
        description=f"Venta registrada",
        lines=revenue_lines,
    ...
    if cost > 0:
        cost_lines = [
            {"puc_code": "6135", "debit": float(cost), "credit": 0, "description": "Costo de mercancía vendida"},
            {"puc_code": "1435", "debit": 0, "credit": float(cost), "description": "Salida de inventario"},
        ]
        entry2 = create_journal_entry(
            tenant_id=tenant_id,
            created_by=created_by,
            entry_type="SALE_COST",
            description=f"Costo de venta",
            lines=cost_lines,
            source_document_type="sale",
            source_document_id=sale_id,
        )
...
def post_sale_reversal(
    ...
    lines = [
        {"puc_code": "4175", "debit": float(sub), "credit": 0, "description": "Devolución en ventas"},
        {"puc_code": cash_account, "debit": 0, "credit": float(total), "description": "Devolución al cliente"},
    ]
    if tax > 0:
        lines.append(
            {
    ...
    if cost > 0:
        cost_lines = [
            ...
            {"puc_code": "6135", "debit": 0, "credit": float(cost), "description": "Reversa costo de venta"},
        ]
        entry2 = create_journal_entry(
            tenant_id=tenant_id,
            created_by=created_by,
...
def post_sale_credit_note_entry(
    ...
    lines = [
        {"puc_code": "4175", "debit": float(sub), "credit": 0, "description": "Devolución parcial en ventas"},
        {"puc_code": cash_account, "debit": 0, "credit": float(total), "description": "Reembolso al cliente"},
    ]
    if tax > 0:
        lines.append(
            {
    ...
    entry1 = create_journal_entry(
        ...
        created_by=created_by,
        entry_type="REVERSAL",
        description="Nota crédito de venta",
        lines=lines,
    ...
    if cost > 0:
        ...
        entry2 = create_journal_entry(
            ...
            created_by=created_by,
            entry_type="REVERSAL",
            description="Reversa costo - nota crédito",
            lines=cost_lines,
            source_document_type="credit_note",
            source_document_id=credit_note_id,
        )
...
def create_expense(
    ...
    # Accounting entry
    lines = [
        {"puc_code": puc_code, "debit": float(amt), "credit": 0, "description": concept},
    ]
    if tax > 0:
        lines.append({"puc_code": "2408", "debit": float(tax), "credit": 0, "description": "IVA descontable"})
    ...
    for w in withholdings:
        lines.append(
            {"puc_code": w["puc_code"], "debit": 0, "credit": w["amount"], "description": f"{w['name']} ({w['rate']}%)"}
        )
    ...
    create_journal_entry(
        ...
        source_document_id=str(expense.id),
    )
...
def pay_expense(tenant_id: str, expense_id: str, user_id: str, payment_method: str = "cash") -> dict:
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": "2335", "debit": net_payable, "credit": 0, "description": "Pago gasto causado"},
            {
                "puc_code": cash_account,
                "debit": 0,
                "credit": net_payable,
...
def monthly_close(tenant_id: str, year: int, month: int, user_id: str) -> dict:
    ...
    return {
        ...
        "message": (
            f"Cierre anual {year} completado. Asiento contable generado."
            if is_annual
            else f"Periodo {year}-{month:02d} cerrado. No se pueden crear movimientos en este mes."
        ),
    }
...
def get_inventory_accounting_balance(tenant_id: str) -> float:
    ...
    account = ChartOfAccount.query.filter_by(tenant_id=tenant_id, puc_code="1435").first()
    if not account:
        return 0.0
...
def get_inventory_physical_value(tenant_id: str) -> float:
    ...
    products = Product.query.filter_by(tenant_id=tenant_id, is_active=True).filter(Product.deleted_at.is_(None)).all()
    total = sum(float(p.stock_current) * float(p.cost_average or p.purchase_price or 0) for p in products)
...
def create_opening_balance(
    ...
    existing = JournalEntry.query.filter_by(tenant_id=tenant_id, entry_type="OPENING").first()
    if existing:
    ...
    if inventory_to_book > 0:
        lines.append(
            {
                "puc_code": "1435",
                "debit": inventory_to_book,
                "credit": 0,
                "description": "Inventario inicial (calculado)",
            }
        )
    if receivables > 0:
        lines.append(
            {"puc_code": "1305", "debit": receivables, "credit": 0, "description": "Saldo inicial clientes (CxC)"}
        )
    if payables > 0:
        lines.append(
            {"puc_code": "2205", "debit": 0, "credit": payables, "description": "Saldo inicial proveedores (CxP)"}
        )
    if capital > 0:
    ...
    if retained_earnings > 0:
        lines.append(
            {"puc_code": "3710", "debit": 0, "credit": retained_earnings, "description": "Utilidades acumuladas"}
        )
...
def get_withholdings(tenant_id: str) -> list:
    configs = WithholdingConfig.query.filter_by(tenant_id=tenant_id, is_active=True).all()
    return [
        {
            "id": str(c.id),
            "type": c.type,
...
def create_account(
    ...
    existing = ChartOfAccount.query.filter_by(tenant_id=tenant_id, puc_code=puc_code).first()
    if existing:
...
def update_account(tenant_id: str, account_id: str, **kwargs) -> dict:
    ...
    account = ChartOfAccount.query.filter_by(id=account_id, tenant_id=tenant_id).first()
    if not account:
...
def seed_chart_of_accounts(tenant_id: str) -> int:
    ...
    existing_codes = {row.puc_code for row in ChartOfAccount.query.filter_by(tenant_id=tenant_id).all()}
...
def post_voucher_sale_entry(
    ...
    return create_journal_entry(
        ...
        source_document_id=sale_id,
        lines=[
            {"puc_code": "1105", "debit": amount, "credit": 0, "description": "Cobro venta de bono"},
            {"puc_code": "291001", "debit": 0, "credit": amount, "description": "Obligación por bono vendido"},
        ],
    )
...
def post_voucher_redemption_entry(
    ...
    """
    lines = [
        {"puc_code": "291001", "debit": amount, "credit": 0, "description": "Liberación pasivo por bono redimido"},
    ]

    if tax_amount > 0:
        net_income = amount - tax_amount
        lines.append(
            {"puc_code": "4135", "debit": 0, "credit": net_income, "description": "Ingreso por redención de bono"}
        )
        lines.append(
            {"puc_code": "2408", "debit": 0, "credit": tax_amount, "description": "IVA sobre productos de la venta"}
        )
    else:
        lines.append({"puc_code": "4135", "debit": 0, "credit": amount, "description": "Ingreso por redención de bono"})
...
def post_voucher_expiry_entry(
    ...
    if quarantine:
        return create_journal_entry(
            ...
            lines=[
                {"puc_code": "291001", "debit": amount, "credit": 0, "description": "Liberación pasivo bono expirado"},
                {"puc_code": "291002", "debit": 0, "credit": amount, "description": "Bono en cuarentena (30 días)"},
            ],
        )
    else:
        return create_journal_entry(
            ...
            source_document_id=voucher_id,
            lines=[
                {"puc_code": "291002", "debit": amount, "credit": 0, "description": "Salida de cuarentena"},
                {
                    "puc_code": "429505",
                    "debit": 0,
                    "credit": amount,
...
def get_chart_of_accounts(tenant_id: str) -> list:
    ...
    accounts = ChartOfAccount.query.filter_by(tenant_id=tenant_id).order_by(ChartOfAccount.puc_code).all()
...
def get_journal_entries(
    ...
    total = q.count()
    entries = q.order_by(JournalEntry.created_at.desc()).offset((page - 1) * per_page).limit(per_page).all()
