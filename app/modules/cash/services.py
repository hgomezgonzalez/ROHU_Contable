...
def _next_number(model, prefix, tenant_id):
    ...
    last = db.session.query(func.max(col)).filter(model.tenant_id == tenant_id, col.like(f"{full_prefix}%")).scalar()
...
def create_cash_receipt(
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": debit_account, "debit": float(amt), "credit": 0, "description": f"Ingreso - {concept}"},
            {"puc_code": credit_account, "debit": 0, "credit": float(amt), "description": source_name or concept},
        ],
...
def void_cash_receipt(tenant_id: str, receipt_id: str, user_id: str) -> dict:
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": credit_account, "debit": float(receipt.amount), "credit": 0, "description": "Reversa ingreso"},
            {
                "puc_code": debit_account,
                "debit": 0,
                "credit": float(receipt.amount),
                "description": "Reversa caja/banco",
            },
        ],
        source_document_type="cash_receipt_void",
        source_document_id=str(receipt.id),
    )
...
def create_cash_disbursement(
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": debit_account, "debit": float(amt), "credit": 0, "description": concept},
            {
                "puc_code": credit_account,
                "debit": 0,
                "credit": float(amt),
                "description": f"Egreso - {disb.disbursement_number}",
            },
        ],
        source_document_type="cash_disbursement",
        source_document_id=str(disb.id),
    )
...
def void_cash_disbursement(tenant_id: str, disb_id: str, user_id: str) -> dict:
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": credit_account, "debit": float(disb.amount), "credit": 0, "description": "Reversa salida"},
            {"puc_code": debit_account, "debit": 0, "credit": float(disb.amount), "description": "Reversa gasto/pago"},
        ],
        source_document_type="cash_disbursement_void",
        source_document_id=str(disb.id),
    )
...
def create_cash_transfer(
    ...
    transfer = CashTransfer(
        ...
        from_account_puc=from_account_puc,
        to_account_puc=to_account_puc,
        amount=amt,
        reference=reference,
        notes=notes,
    )
    ...
    create_journal_entry(
        ...
        lines=[
            {"puc_code": to_account_puc, "debit": float(amt), "credit": 0, "description": f"Ingreso desde {from_name}"},
            {"puc_code": from_account_puc, "debit": 0, "credit": float(amt), "description": f"Traslado a {to_name}"},
        ],
        source_document_type="cash_transfer",
        source_document_id=str(transfer.id),
    )
...
def get_cash_transfers(tenant_id: str) -> list:
    transfers = CashTransfer.query.filter_by(tenant_id=tenant_id).order_by(CashTransfer.created_at.desc()).all()
